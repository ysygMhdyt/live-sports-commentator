...
var TranscriptionManager = class {
  constructor() {
    ...
    this.mergeThreshold = 2e3;
  }
  addTranscription(text, timestamp) {
    const currentTime = new Date(timestamp);
    if (this.lastTimestamp && currentTime - this.lastTimestamp < this.mergeThreshold && this.transcriptions.length > 0) {
...
var transcriptionManager = new TranscriptionManager();
var transcriptionManager_default = transcriptionManager;
  ...
  if (typeof process !== "undefined" && ((_a2 = process == null ? void 0 : process.env) == null ? void 0 : _a2["DEBUG"]) === "true") {
    const modifiedArgs = args.map((arg) => {
      ...
      for (const header in arg) {
        if (SENSITIVE_HEADERS.has(header.toLowerCase())) {
          modifiedArg != null ? modifiedArg : modifiedArg = __spreadValues({}, arg);
...
var ResponseStream = class _ResponseStream extends EventStream {
  ...
  static createResponse(client, params, options) {
    ...
    runner._run(() => runner._createResponse(client, params, __spreadProps(__spreadValues({}, options), {
      headers: __spreadProps(__spreadValues({}, options == null ? void 0 : options.headers), { "X-Stainless-Helper-Method": "stream" })
    })));
    return runner;
  }
  async _createResponse(client, params, options) {
    ...
    const stream = await client.responses.create(__spreadProps(__spreadValues({}, params), { stream: true }), __spreadProps(__spreadValues({}, options), { signal: this.controller.signal }));
    this._connected();
    try {
      for (var iter = __forAwait(stream), more, temp, error; more = !(temp = await iter.next()).done; more = false) {
        const event = temp.value;
    ...
    switch (event.type) {
      case "response.output_text.delta": {
        ...
        if (output.type === "message") {
          ...
          this._emit("response.output_text.delta", __spreadProps(__spreadValues({}, event), {
      ...
      case "response.function_call_arguments.delta": {
        ...
        if (output.type === "function_call") {
          this._emit("response.function_call_arguments.delta", __spreadProps(__spreadValues({}, event), {
...
var Responses = class extends APIResource {
  ...
  create(body, options) {
    var _a2;
    return this._client.post("/responses", __spreadProps(__spreadValues({ body }, options), { stream: (_a2 = body.stream) != null ? _a2 : false }))._thenUnwrap((rsp) => {
  ...
  retrieve(responseId, query = {}, options) {
    ...
    return this._client.get(`/responses/${responseId}`, __spreadValues({ query }, options));
  }
  /**
  ...
  del(responseId, options) {
    return this._client.delete(`/responses/${responseId}`, __spreadProps(__spreadValues({}, options), {
      headers: __spreadValues({ Accept: "*/*" }, options == null ? void 0 : options.headers)
    }));
  }
  parse(body, options) {
...
var Files2 = class extends APIResource {
  ...
  content(vectorStoreId, fileId, options) {
    return this._client.getAPIList(`/vector_stores/${vectorStoreId}/files/${fileId}/content`, FileContentResponsesPage, __spreadProps(__spreadValues({}, options), { headers: __spreadValues({ "OpenAI-Beta": "assistants=v2" }, options == null ? void 0 : options.headers) }));
  }
};
...
var openai = new openai_default({
  ...
  apiKey: "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"
});
...
async function sendToWhisper(audioData) {
  ...
  try {
    ...
    if (result.text && result.text.trim()) {
      ...
      transcriptionManager_default.addTranscription(
        processedText,
